...
export default async function verifyRecaptcha(
    ...
    try {
        ...
        if (response.tokenProperties.action !== recaptchaAction) {
            console.warn("L'action attendue n'a pas été exécutée.")
            return false
        }
        ...
        if (response.riskAnalysis) {
            ...
            if (score !== undefined && score !== null) {
                console.warn(`Le score reCAPTCHA est : ${score}`)

                // Vérification de l'existence et du type de 'reasons'
                if (Array.isArray(response.riskAnalysis.reasons)) {
                    response.riskAnalysis.reasons.forEach(reason => {
                        console.warn(reason) // Tu peux loguer tout l'objet 'reason' ou afficher des propriétés spécifiques
                    })
                }
                ...
                console.warn('Le score reCAPTCHA est introuvable ou nul.')
                return false
            }
        }

        console.warn("L'évaluation du risque (riskAnalysis) est introuvable.")
        return false
    } catch (error) {
