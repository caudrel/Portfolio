...
class ProjectsResolver {
  ...
  async projects(@Arg("technologyIds", () => [Int], { nullable: true }) technologyIds?: number[]): Promise<ProjectCard[]> {
    ...
    if (technologyIds && technologyIds.length > 0) {
      query.innerJoin("project.technologies", "filterTech", "filterTech.id IN (:...technologyIds)", { technologyIds });
    }
  ...
  @Mutation(() => Project)
  async createProject(@Arg("data", { validate: true }) data: ProjectInput): Promise<Project> {
    let {
      title,
      completion_date,
      duration,
